class Buzzer extends AudioWorkletProcessor {
    constructor(...args) {
        ...
        let b1f = false, b2f = false

        let value = 0
        let readValue = 0
        let readValueTimestamp = 0

        this.port.onmessage = ({ data: {
            portForAudio
        } }) => {
            if (portForAudio !== undefined) {
                this.anotherPort = portForAudio
                this.anotherPort.onmessage = ({ data }) => {
                    if (buffer === null) {
                        buffer = new Float64Array(data.length * 2)
                    }
                    currentBuffer = data
                    const offset = bufferPtr < data.length ? data.length : 0
                    for (let i = 0; i < data.length; i++) {
                        buffer[i + offset] = data[i]
                    }
                    if (bufferPtr < data.length) {
                        b2f = true
                    } else {
                        b1f = true
                    }
        ...
        this.advance = (
            ...
            let targetTimestamp = timestamp + deltaTime

            while (readValueTimestamp < targetTimestamp) {
                if (readValueTimestamp > timestamp) {
                    const dt = readValueTimestamp - timestamp
                    shiftState(dt)
                    addImpulse((readValue - value) * gain)

                    timestamp = readValueTimestamp
                    value = readValue
                }

                const raw = buffer ? buffer[bufferPtr] : 0
                if (Math.abs(raw) < 1) {
                    if (currentBuffer !== null) {
                        this.anotherPort.postMessage(currentBuffer, [currentBuffer.buffer])
                        currentBuffer = null
                    }
                    if (!b1f && !b2f) {
                        break
                    }
                    if (buffer && bufferPtr < buffer.length / 2) {
                        bufferPtr = buffer.length / 2
                        b2f = false
                    } else if (buffer) {
                        bufferPtr = 0
                        b1f = false
                ...
                readValue = raw > 0 ? 1 : 0
                readValueTimestamp = Math.abs(raw) - 1
            }

            const dt = targetTimestamp - timestamp
            shiftState(dt)
