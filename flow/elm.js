...
Elm.Flow = Elm.Flow || {};
Elm.Flow.make = function (_elm) {
   "use strict";
   _elm.Flow = _elm.Flow || {};
   if (_elm.Flow.values) return _elm.Flow.values;
   var _U = Elm.Native.Utils.make(_elm),
   $Basics = Elm.Basics.make(_elm),
   $Char = Elm.Char.make(_elm),
   $Debug = Elm.Debug.make(_elm),
   ...
   var _op = {};
   var TimeElapsed = function (a) {    return {ctor: "TimeElapsed",_0: a};};
   var ticks = A2($Signal.map,TimeElapsed,$Time.fps(5));
   var NewParagraph = {ctor: "NewParagraph"};
   var newParagraphs = A2($Signal.map,function (_p0) {    return NewParagraph;},$Keyboard.enter);
   var Typed = function (a) {    return {ctor: "Typed",_0: a};};
   var spaceBars = A2($Signal.map,function (_p1) {    return Typed(" ");},$Keyboard.space);
   var codeToAction = function (code) {    return Typed(function (_p2) {    return $String.trim($String.fromChar($Char.fromCode(_p2)));}(code));};
   var chars = A2($Signal.map,codeToAction,$Keyboard.presses);
   var actions = $Signal.mergeMany(_U.list([newParagraphs,spaceBars,chars,ticks]));
   var viewTime = function (t) {
      var dd = function (_p3) {    return A3($String.padLeft,2,_U.chr("0"),$Basics.toString(_p3));};
      var minutes = $Basics.floor(t / $Time.minute);
      var seconds = A2($Basics._op["%"],$Basics.floor(t / $Time.second),60);
      return $Html.text(A2($Basics._op["++"],dd(minutes),A2($Basics._op["++"],":",dd(seconds))));
   };
   var defaultTimeToFade = 5 * $Time.second;
   var viewText = F2(function (model,timeToFade) {
      return A2($Html.div,
      _U.list([$Html$Attributes.style(_U.list([{ctor: "_Tuple2",_0: "padding",_1: "20px"}
                                              ,{ctor: "_Tuple2",_0: "font-family",_1: "Modern"}
                                              ,{ctor: "_Tuple2",_0: "font-size",_1: "14pt"}
                                              ,{ctor: "_Tuple2",_0: "opacity",_1: $Basics.toString(timeToFade / defaultTimeToFade)}]))]),
      A2($List.map,function (txt) {    return A2($Html.p,_U.list([]),_U.list([$Html.text(txt)]));},model));
   });
   var view = function (_p4) {
      var _p5 = _p4;
      return A2($Html.div,_U.list([]),_U.list([viewTime(_p5.sessionTimeLeft),A2(viewText,_p5.model,_p5.timeToFade)]));
   };
   var update = F2(function (action,m) {
      var timeToFade = m.timeToFade;
      var model = m.model;
      var reversed = $List.reverse(model);
      if (_U.cmp(m.sessionTimeLeft,0) > 0) {
            var _p6 = {ctor: "_Tuple2",_0: action,_1: reversed};
            switch (_p6._0.ctor)
            {case "Typed": if (_p6._1.ctor === "[]") {
                       return _U.update(m,{model: _U.list([_p6._0._0]),timeToFade: defaultTimeToFade});
                    } else {
                       return _U.update(m,
                       {model: $List.reverse(A2($List._op["::"],A2($Basics._op["++"],_p6._1._0,_p6._0._0),_p6._1._1)),timeToFade: defaultTimeToFade});
                    }
               case "NewParagraph": if (_p6._1.ctor === "[]") {
                       return _U.update(m,{model: _U.list([]),timeToFade: defaultTimeToFade});
                    } else {
                       if (_p6._1._0 === "") {
                             return m;
                          } else {
                             return _U.update(m,{model: A2($Basics._op["++"],model,_U.list([""])),timeToFade: defaultTimeToFade});
                          }
                    }
               default: var _p7 = _p6._0._0;
                 return _U.cmp(_p7,timeToFade) > 0 ? _U.update(m,{model: _U.list([]),timeToFade: 0,sessionTimeLeft: m.sessionTimeLeft - _p7}) : _U.update(m,
                 {timeToFade: timeToFade - _p7,sessionTimeLeft: m.sessionTimeLeft - _p7});}
         } else return _U.update(m,{timeToFade: defaultTimeToFade,sessionTimeLeft: 0});
   });
   var state = A3($Signal.foldp,update,{model: _U.list([]),timeToFade: defaultTimeToFade,sessionTimeLeft: 5 * $Time.minute},actions);
   var main = A2($Signal.map,view,state);
   return _elm.Flow.values = {_op: _op
                             ,main: main
                             ,defaultTimeToFade: defaultTimeToFade
                             ,view: view
                             ,viewText: viewText
                             ,viewTime: viewTime
                             ,state: state
